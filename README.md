...
Built on top of ONNX Runtime
 and the ONNX protobuf API
, it operates directly at the graph level — allowing weight manipulation that a standard inference session cannot expose.
...
You can visually inspect the model using Netron
. Look for QuantizeLinear and DequantizeLinear nodes, and check that weight initializers have INT8 data type.
